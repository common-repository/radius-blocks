...
$wrap_class .= ' rtrb-block rtrb-social-share rtrb-block-frontend';
				...
				?>
					...
					<?php if (!empty($socialicon['icon']) || !empty($socialicon['image']['url'])) : ?>
						<li class="rtrb-social-item">
							<a style="<?php echo esc_attr($style); ?>" href="<?php echo Fns::rtrb_social_share_link($post->ID, $socialicon['title']);  ?>" class="rtrb-share-btn rtrb-social-link <?php echo strtolower('rt-' . $socialicon['title']); ?>" target="_blank" rel="nofollow noopener noreferrer">

								<?php if ($socialicon['iconType'] == 'icon') : ?>
									<span class="rtrb-social-icon"><?php echo Fns::render_svg_html($socialicon['icon']); ?></span>
								<?php endif; ?>

								<?php if ($socialicon['iconType'] == 'image') : ?>
