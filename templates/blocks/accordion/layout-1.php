...
if (isset($settings['layout']) && !empty($settings['layout'])) {
	$block_wrap_class .= ' rtrb-accordion--style-' . $settings['layout'];
}
?>
					...
					<div class="rtrb-accordion__item <?php echo esc_attr($exp_tab); ?>">
						...
						<div class="rtrb-accordion__header">
							<div class="rtrb-accordion-title-wrap">
								...
								<?php
								if ($settings['displayTitleIcon'] && !empty($accordion['icon'])) :
									$tabTitleIcon = Fns::render_svg_html($accordion['icon']);
								?>
									<span class="rt-icon">
										<?php echo Fns::rtrb_kses_intermediate($tabTitleIcon);  ?>
									</span>
								<?php endif; ?>

								<?php if (!empty($accordion['title'])) : ?>
									<?php echo '<' . esc_html($settings['tabTitleTagName']) . ' class="rtrb-accordion-title rtrb-tag">'; ?>
									<?php echo Fns::rtrb_kses_basic($accordion['title']); ?>
									<?php echo '</' . esc_html($settings['tabTitleTagName']) . '>' ?>
								<?php endif; ?>
							</div>

							<?php if ($settings['displayIcon'] && (!empty($tabIcon) || !empty($tabExpandedIcon))) : ?>
								<div class="rt-accordion-icon-wrap">

									<?php if (!empty($tabIcon)) : ?>
										<span class="rt-accordion-icon rt-expand-icon">
											<?php echo Fns::rtrb_kses_intermediate($tabIcon); ?>
										</span>
									<?php endif; ?>

									<?php if (!empty($tabExpandedIcon)) : ?>
										<span class="rt-accordion-icon rt-tab-icon">
											<?php echo Fns::rtrb_kses_intermediate($tabExpandedIcon); ?>
										</span>
									<?php endif; ?>
