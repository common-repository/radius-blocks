...
class SocialShare
{
	...
	public function __construct()
	{
		...
		add_action('init', [$this, 'register_socialshare']);
	}

	public function rtrb_blcoks_css_enqueue()
	{
		if (Fns::rtrb_has_block('rtrb/social-share') && !is_admin()) {
	...
	public function register_socialshare()
	{
		register_block_type(
			RTRB_PATH_BLOCKS . 'social-share',
			[
	...
	public function render_block($settings)
	{
		...
		$data = [
			'template'              => 'blocks/social-share/layout-' . $layout,
			'settings'              => $settings,
			'post'					=> $post,
			'default_template_path' => null,
		];
		$data = apply_filters('rtrb_social_share_block_data', $data);
