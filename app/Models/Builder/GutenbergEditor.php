...
class GutenbergEditor extends BuilderCompatibility
{
	...
	public function render_content()
	{
		...
		$output       = '';
		$current_post = get_post($this->post_id, OBJECT);
	...
	public function enqueue_scripts()
	{
		...
		if ($post_id !== '') {
			...
			if (class_exists('UAGB_Post_Assets')) {
				...
				if (!empty($ultimate_instance) && is_callable([$ultimate_instance, 'enqueue_scripts'])) {
			...
			if (class_exists('Gutentor_Dynamic_CSS')) {
				...
				if (!empty($gutentor_instance) && is_callable([
					$gutentor_instance,
					'get_singular_dynamic_css'
				])) {
					global $post;
			...
			if (class_exists('EbStyleHandler')) {
				...
				if (!empty($essential_instance) && is_callable([
					$essential_instance,
